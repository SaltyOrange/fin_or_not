...
def inference(batch_size):
    ...
    conv1_pool_norm = tf.nn.local_response_normalization(
        conv1_pool, name="conv1_pool_norm")

    conv2_1 = conv_layer(conv1_pool_norm, [3, 3, 16, 16], "conv2_1")
    ...
    conv2_pool_norm = tf.nn.local_response_normalization(
        conv2_pool, name="conv2_pool_norm")

    conv3_1 = conv_layer(conv2_pool_norm, [3, 3, 32, 32], "conv3_1")
    ...
    conv3_pool_norm = tf.nn.local_response_normalization(
        conv3_pool, name="conv3_pool_norm")
    
    gap = tf.reduce_mean(conv3_pool_norm, [1, 2])
